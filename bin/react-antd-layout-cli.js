    ...
    request(
      {
        ...
        headers: {
          Authorization:
            'token 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',
...
let templateCreatSuccess = false;
...
async function getFileContent(list = templateFolders) {
  ...
  /* 重复请求超过5次就直接提示仓库克隆项目 */
  if (errorRequestCount > 5 && list.length) {
  ...
  if (errorRequestFiles.length) {
    await getFileContent(errorRequestFiles);
  } else {
    templateCreatSuccess = true;
  }
}
createProject();
